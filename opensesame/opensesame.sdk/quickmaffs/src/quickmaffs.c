...
int main(){
	...
	while(1){
		...
		for(addr=QMAXI_S00_AXI_SLV_REG1_OFFSET; addr<=QMAXI_S00_AXI_SLV_REG128_OFFSET;addr=addr+4){
			QMAXI_mWriteReg(XPAR_QMAXI_0_S00_AXI_BASEADDR, addr, raw_32[i++]);
		}
		sleep(1);
		i=0;
		for(addr=QMAXI_S00_AXI_SLV_REG129_OFFSET; addr<=QMAXI_S00_AXI_SLV_REG256_OFFSET;addr=addr+4){
...
//		i=0;
//		for(addr=QMAXI_S00_AXI_SLV_REG1_OFFSET; addr<=QMAXI_S00_AXI_SLV_REG4_OFFSET;addr=addr+4){
//			QMAXI_mWriteReg(XPAR_QMAXI_0_S00_AXI_BASEADDR, addr, raw_32[i]);
//			i++;
//		}
//		sleep(1);
//		i=0;
//		for(addr=QMAXI_S00_AXI_SLV_REG129_OFFSET; addr<=QMAXI_S00_AXI_SLV_REG132_OFFSET;addr=addr+4){
//			out_32[i]=QMAXI_mReadReg(XPAR_QMAXI_0_S00_AXI_BASEADDR, addr);
//			i++;
//		}
//		xil_printf("%d , %d, %d, %d \n",out_32[0],out_32[1],out_32[2],out_32[3]);

//		QMAXI_mWriteReg(XPAR_QMAXI_0_S00_AXI_BASEADDR, QMAXI_S00_AXI_SLV_REG102_OFFSET,send);
//		sleep(1);
//		send++;
//		recv=QMAXI_mReadReg(XPAR_QMAXI_0_S00_AXI_BASEADDR, QMAXI_S00_AXI_SLV_REG230_OFFSET);
//		xil_printf("%d \n\r",recv);
		//print("extra wire\n");
